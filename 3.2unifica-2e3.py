...
# -----------------------------
# Primeira etapa: preencher padrão inicial
# -----------------------------
for row in range(2, ws.max_row+1):
    ...
    # Ajuste de férias
    if c_val and em_ferias(c_val, data):
        c_nome = ""
    if d_val and em_ferias(d_val, data):
        d_nome = ""
    ...
    if dia_semana < 4:  # segunda a quinta
        ...
        if c_nome: contagem_total[c_nome]["Diurno Semana"] +=1
        if d_nome: contagem_total[d_nome]["Noturno FimSemana"] +=1  # ✅ sexta noturno conta como FDS
...
# -----------------------------
# Segunda etapa: preencher lacunas FDS
# -----------------------------
for row in range(2, ws.max_row+1):
    data = parse_data(ws.cell(row=row, column=col_data).value)
    if not data: continue
