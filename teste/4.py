...
def aparece_na_linha(ws, row, nome, col_diurno, col_noturno):
    if row < 2 or row > ws.max_row:
        return False
    return (ws.cell(row=row, column=col_diurno).value == nome) or \
           (ws.cell(row=row, column=col_noturno).value == nome)

# --------------------------------------------------------------
# FUNÇÃO NOVA — ALOCAÇÃO INTELIGENTE COM REALOCAÇÃO + TROCA
# --------------------------------------------------------------
def tentar_alocar_com_troca(nome, idx_atual, datas, ws, col, contagem_dict, counts_dict, dias_validos, ferias_exped):

    # --------------------------
    #  Função de validação
    # --------------------------
    def pode_alocar(nome, row):
        mesmo = aparece_na_linha(ws, row, nome, 3, 4)
        ant   = aparece_na_linha(ws, row-1, nome, 3, 4)
        prox  = aparece_na_linha(ws, row+1, nome, 3, 4)
        return not (mesmo or ant or prox)

    # ------------------------------------------------
    # FUNÇÃO registrar — corrigida para evitar KeyError
    # ------------------------------------------------
    def registrar(nome, row, data):
        ws.cell(row=row, column=col, value=nome)

        # Apenas EXPEDIENTES aparecem em counts_dict
        if nome in counts_dict:
            counts_dict[nome] += 1

        # Atualiza a aba Resumo para TODOS (plantonistas + expedientes)
    ...
    # -----------------------------------------------------
    # 1) TENTAR NA LINHA ATUAL (SEM REALOCAR)
    # -----------------------------------------------------
    if not ws.cell(row=row_atual, column=col).value:
        if pode_alocar(nome, row_atual) and not in_ferias(nome, data_atual, ferias_exped):
            registrar(nome, row_atual, data_atual)
            return True

    # -----------------------------------------------------
    # 2) REALOCAR PARA FRENTE
    # -----------------------------------------------------
    for fut_idx in range(idx_atual+1, len(datas)):
        fut_row, fut_data = datas[fut_idx]
        if fut_data.weekday() in dias_validos:
            if not ws.cell(row=fut_row, column=col).value:
                if pode_alocar(nome, fut_row) and not in_ferias(nome, fut_data, ferias_exped):
                    registrar(nome, fut_row, fut_data)
                    return True

    # -----------------------------------------------------
    # 3) REALOCAR PARA TRÁS SEM TROCA
    # -----------------------------------------------------
    for prev_idx in range(idx_atual-1, -1, -1):
        prev_row, prev_data = datas[prev_idx]
        if prev_data.weekday() in dias_validos:
            if not ws.cell(row=prev_row, column=col).value:
                if pode_alocar(nome, prev_row) and not in_ferias(nome, prev_data, ferias_exped):
                    registrar(nome, prev_row, prev_data)
                    return True

    # -----------------------------------------------------
    # 4) TROCA INTELIGENTE — APENAS ENTRE EXPEDIENTES
    # -----------------------------------------------------
    for prev_idx in range(idx_atual-1, -1, -1):
        prev_row, prev_data = datas[prev_idx]
        ...
        ocupante = ws.cell(row=prev_row, column=col).value
        if not ocupante:
            continue

        # ❌ PROIBIR TROCA COM PLANTONISTA
        if ocupante not in counts_dict:  # se não está em counts_dict → não é expediente
            continue

        # Ocupante não pode estar de férias no dia da troca
        if in_ferias(ocupante, data_atual, ferias_exped):
            continue

        # Ambos precisam respeitar as regras
        if pode_alocar(nome, prev_row) and pode_alocar(ocupante, row_atual):

            # ---------- FAZ TROCA ----------
            ws.cell(row=prev_row, column=col, value=nome)
            ws.cell(row=row_atual, column=col, value=ocupante)

            # Registrar contagens revisadas
            registrar(nome, prev_row, prev_data)
            registrar(ocupante, row_atual, data_atual)
...
# --------------------------------------------------------------
# ETAPA 1 – PLANTONISTAS (seu código original)
# --------------------------------------------------------------
start_rr = 0
...
# --------------------------------------------------------------
# ETAPA 2 – EXPEDIENTES NOTURNOS FIM DE SEMANA
# --------------------------------------------------------------
...
# --------------------------------------------------------------
# ETAPA 3 – NOTURNOS DE SEMANA (AGORA COM TROCA)
# --------------------------------------------------------------
for idx, (row, data) in enumerate(datas):
        ...
        if not ws_out.cell(row=row, column=COL_NOTURNO).value:
            ...
            if disponiveis:
                ...
                for nome in candidatos:
                    if tentar_alocar_com_troca(
                        nome, idx, datas, ws_out, COL_NOTURNO,
                        contagem, counts_exped, (0,1,2,3), ferias_exped
                    ):
                        break
...
for idx, (row, data) in enumerate(datas):
    if data.weekday() in (5,6):
        if not ws_out.cell(row=row, column=COL_DIURNO).value:
            ...
            if disponiveis:
                ...
                for nome in candidatos:
                    if tentar_alocar_com_troca(
                        nome, idx, datas, ws_out, COL_DIURNO,
                        contagem, counts_exped, (5,6), ferias_exped
                    ):
                        break

# --------------------------------------------------------------
# ETAPA 4B – DIURNOS DE SEMANA (COM TROCA)
# --------------------------------------------------------------
for idx, (row, data) in enumerate(datas):
        ...
        if not ws_out.cell(row=row, column=COL_DIURNO).value:
            ...
            if disponiveis:
                ...
                for nome in candidatos:
                    if tentar_alocar_com_troca(
                        nome, idx, datas, ws_out, COL_DIURNO,
                        contagem, counts_exped, (0,1,2,3,4), ferias_exped
                    ):
                        break
