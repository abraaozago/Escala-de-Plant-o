...
# -------------------------------
# Etapa 2: Expedientes nos noturnos de fim de semana
# -------------------------------
...
# -------------------------------
# Etapa 3: Expedientes nos noturnos de semana (seg-qui, com realocação)
# -------------------------------
for idx, (row, data) in enumerate(datas):
    if data.weekday() in (0,1,2,3):  # seg-qui
        if not ws_out.cell(row=row, column=COL_NOTURNO).value:
            ...
            if disponiveis:
                # Seleciona quem tem menor carga
                ...
                for nome in candidatos:
                    # Verificar restrição
                    mesmo_dia = ws_out.cell(row=row, column=COL_DIURNO).value == nome
                    anterior = ws_out.cell(row=row-1, column=COL_NOTURNO).value == nome if row > 2 else False
                    posterior = ws_out.cell(row=row+1, column=COL_NOTURNO).value == nome if row < ws_out.max_row else False

                    if not (mesmo_dia or anterior or posterior):
                        # Pode alocar aqui
                        ws_out.cell(row=row, column=COL_NOTURNO, value=nome)
                        counts_exped[nome] += 1
                        ...
                        # Se não pode aqui, tenta realocar em outra lacuna futura
                        for fut_idx in range(idx+1, len(datas)):
                            fut_row, fut_data = datas[fut_idx]
                            if fut_data.weekday() in (0,1,2,3):  # só semana
                                if not ws_out.cell(row=fut_row, column=COL_NOTURNO).value:
                                    mesmo_dia_fut = ws_out.cell(row=fut_row, column=COL_DIURNO).value == nome
                                    anterior_fut = ws_out.cell(row=fut_row-1, column=COL_NOTURNO).value == nome if fut_row > 2 else False
                                    posterior_fut = ws_out.cell(row=fut_row+1, column=COL_NOTURNO).value == nome if fut_row < ws_out.max_row else False
                                    if not (mesmo_dia_fut or anterior_fut or posterior_fut):
                                        ws_out.cell(row=fut_row, column=COL_NOTURNO, value=nome)
                                        counts_exped[nome] += 1
                                        contagem[nome]["Noturno Semana"] += 1
                                        alocado = True
                                        break
                        if alocado:
                            break
# -------------------------------
# Etapa 4a: Expedientes nos diurnos de fim de semana (sab/dom, com realocação)
# -------------------------------
for idx, (row, data) in enumerate(datas):
    if data.weekday() in (5,6):  # sábado/domingo
        if not ws_out.cell(row=row, column=COL_DIURNO).value:
            ...
            if disponiveis:
                ...
                alocado = False
                for nome in candidatos:
                    mesmo_dia = ws_out.cell(row=row, column=COL_NOTURNO).value == nome
                    anterior = ws_out.cell(row=row-1, column=COL_DIURNO).value == nome if row > 2 else False
                    posterior = ws_out.cell(row=row+1, column=COL_DIURNO).value == nome if row < ws_out.max_row else False
                    ...
                    if not (mesmo_dia or anterior or posterior):
                        ...
                        # tenta realocar em outra lacuna futura
                        for fut_idx in range(idx+1, len(datas)):
                            fut_row, fut_data = datas[fut_idx]
                            if fut_data.weekday() in (5,6):
                                if not ws_out.cell(row=fut_row, column=COL_DIURNO).value:
                                    mesmo_dia_fut = ws_out.cell(row=fut_row, column=COL_NOTURNO).value == nome
                                    anterior_fut = ws_out.cell(row=fut_row-1, column=COL_DIURNO).value == nome if fut_row > 2 else False
                                    posterior_fut = ws_out.cell(row=fut_row+1, column=COL_DIURNO).value == nome if fut_row < ws_out.max_row else False
                                    if not (mesmo_dia_fut or anterior_fut or posterior_fut):
                                        ws_out.cell(row=fut_row, column=COL_DIURNO, value=nome)
                                        counts_exped[nome] += 1
                                        contagem[nome]["Diurno FimSemana"] += 1
                                        alocado = True
                                        break
...
# -------------------------------
# Etapa 4b: Expedientes nos diurnos de semana (seg-sex, com realocação)
# -------------------------------
for idx, (row, data) in enumerate(datas):
    if data.weekday() in (0,1,2,3,4):  # seg a sex
        if not ws_out.cell(row=row, column=COL_DIURNO).value:
            disponiveis = [n for n in expedientes if not in_ferias(n, data, ferias_exped)]
            if disponiveis:
                min_total = min(counts_exped[n] for n in disponiveis)
                ...
                for nome in candidatos:
                    ...
                    if not (mesmo_dia or anterior or posterior):
                        ...
                        for fut_idx in range(idx+1, len(datas)):
                            fut_row, fut_data = datas[fut_idx]
                            if fut_data.weekday() in (0,1,2,3,4):
                                if not ws_out.cell(row=fut_row, column=COL_DIURNO).value:
                                    mesmo_dia_fut = ws_out.cell(row=fut_row, column=COL_NOTURNO).value == nome
                                    anterior_fut = ws_out.cell(row=fut_row-1, column=COL_DIURNO).value == nome if fut_row > 2 else False
                                    posterior_fut = ws_out.cell(row=fut_row+1, column=COL_DIURNO).value == nome if fut_row < ws_out.max_row else False
